...
interface UpdateWarriorFormProps {
  warrior: Warrior;
}

const UpdateWarriorForm: React.FC<UpdateWarriorFormProps> = ({ warrior }) => {
  const router = useRouter();
  const updateWarrior = trpc.warrior.update.useMutation();
  const form = useForm<z.infer<typeof warriorSchema>>({
    resolver: zodResolver(warriorSchema),
    defaultValues: {
      name: warrior.name,
      age: warrior.age ?? undefined,
      isEvil: warrior.isEvil,
      race: warrior.race,
      skill: warrior.skill ?? undefined,
    },
  });
  async function onSubmit(values: z.infer<typeof warriorSchema>) {
    try {
      // Call the create mutation with the form values
      await updateWarrior.mutateAsync({ ...values, id: warrior.id });
      ...
      toast.error("Failed to update warrior. Please try again.");
...
export default UpdateWarriorForm;
